...
#función auxiliar para generar el teblero
def generar_tablero(filas, columnas):
    total_celdas = filas * columnas
    #Asegurarse de que no pidamos mas cartas de las que ya tenemos
    if total_celdas > len(cartas):
            cartas_para_tablero = random.sample(cartas, len(cartas))#Usamos todas las cartas que tenemos y luego rellenamos si tenemos más
    while len(cartas_para_tablero) < total_celdas:
            cartas_para_tablero.append(random.choice(cartas)) #Rellenamos con cartas aleatorias si es necesario
    else :
            cartas_para_tablero = random.sample(cartas, total_celdas)
            #Barajamos las listas para asegurarnos de que el orden sea aleatorio para cada tablero
    ...
    for r in range(filas):
            ...
            for c in range(columnas):
                    ...
                    nombre_carta = cartas_para_tablero[cartas_index]
                    cartas_index += 1
